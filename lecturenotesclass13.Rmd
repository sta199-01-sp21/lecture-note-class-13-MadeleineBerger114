...
Game: guess how old Jacob Smith is? 
my guess: 32
interval: 25-35 
true age: 30, almost 31 

Downside to using point estimates: tradeoff between accuracy and precision. Unlikely to be correct with one point estimate - Intervals make it more likely to capture correct value. 
...
to be left handed? *5* Would you be surprised if only 3 of them were left handed? *no - 3 is close to 5*
Would you be surprised if 40 of them were left handed? *yes - 40 is way higher than 5 (a lot of variability)*
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-MadeleineBerger114/manhattan.csv")
```
...
20.7, 21.2, 20.7 (different values each time)
...
What does this mean?
This is the interval that 95% of the sample should fall in, so we're 95% confident that the value will be in this interval. (95% bootstrap confidence interval)
...
What does this mean?
95% confident that the true proportion of mice who died are in this interval. 
...
boot_prop2 <- mice %>%
  ...
  specify(response = lived, success = "no") %>%
  generate(reps = 10000, type = "bootstrap") %>%
  calculate(stat = "prop")
```

```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop2, level = .9)
ci_mice_lived
```
...
visualize(boot_prop2) + 
  shade_ci(ci_mice_lived)
```


Give an interpretation of your result.
The vast majority of the count falls within the 90% confidence interval, which likely indicates that many different dosages of the treatment lead to survival. 
